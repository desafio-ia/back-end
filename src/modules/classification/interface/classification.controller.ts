...
export class ClassificationController {
  constructor(private readonly classificationService: ClassificationServicePort) {}
  ...
  async create(req: Request, res: Response): Promise<Response> {
    try {
      ...
      if(!userId){ return res.status(400).json({ message: 'user id é obrigatória.' });}
      ...
      {
        userId, imageBuffer: imageFile.buffer
      });

      return res.status(201).json({
        id: classification.id,
        userId: userId,
        species: classification.nameSpecies,
  ...
  async getById(req: Request<Params>, res: Response): Promise<Response> {
    try {
      ...
      return res.json({
        ...
        nameSpecies: classification.nameSpecies,
